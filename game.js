...
// game
// Same thing about splitting function
// If you create a functions inside function consider using class (decorators and  closures are exceptions)
// If you what to use func approach move all inner functions out of startGame
function startGame(cells) {
  ...
  const clickOn = (event) => {
    ...
    if (isVictory(cells)) {
      newTeg.innerText =
        counter % 2 === 0 ? 'Winner sing "X"' : 'Winner sing "O"';
      elem.append(newTeg);
